...
String[] commonHeader = {"WeekData", "MTYPE", "PROC", "TesterType", "ExecuteDate", "MSS"};
//for (int loop = 0; loop < commonHeader.length ; loop++){
//	commonHeader[loop] = "<td>" + commonHeader[loop] + "</td>";
//}

String[] allDriveHeader = {"AllMean", "All_Batch", "All_Count", "All_FirstYield", "All_LatestYield", "All_sDev",  "All_Min", "All_Max", "All_99", "All_95", "All_100"};
//for (int loop = 0; loop < allDriveHeader.length ; loop++){
//	allDriveHeader[loop] = "<td>" + allDriveHeader[loop] + "</td>";
//}

String[] passerDriveHeader = {"PasserMean", "Passer_Batch", "Passer_Count", "Passer_FirstYield", "Passer_LatestYield", "Passer_sDev", "Passer_Min", "Passer_Max", "Passer_99", "Passer_95", "Passer_100"};
...
for (int entryCount = 0 ; entryCount < mtype2.size(); entryCount++){
	String mtypeName2 = (String)mtype2.get(entryCount);
	  hashTesterType.put(processID.get(processIndex), ttMonitor.getTesterType(mtypeName2, (Integer)processID.get(processIndex)));
	  Vector vtTesterType = (Vector)hashTesterType.get((Integer)processID.get(processIndex));
	  for ( int ttCount = 0 ; ttCount < vtTesterType.size(); ttCount+=1){
		String tester2TypeName = (String)vtTesterType.get(ttCount);	
	%>
	...
	<% Vector vt = ttMonitor.getTestTimeTrend(mtypeName2,(Integer)processID.get(processIndex),tester2TypeName,"ALL_PROD");
		if (vt.size()>0){%>
			...
			<% 
			boolean hilightMean = false;
			if(byMeanTesterType.contains(tester2TypeName)){
				hilightMean = true;%>
			<%} else{
				hilightMean = false;%>
		...
		<% for (int vDataLoop = 0; vDataLoop < vt.size(); vDataLoop++){
			//out.println("mtypeName2: " + mtypeName2 + " , " + "hashTesterType: " + vtTesterType);%>
			<tr align="center">
			...
			<td style="background-color:#22FF00"><%=((TestTimeMonitor) vt.get(vDataLoop)).getMss()%></td>
			
			<%if (reportByAllDrives.equals("checked")){%>
				<%if(hilightMean == true){%>
				<td style="background-color:#22FF00">
				<%}else {%>
				<td>
				<%}%><%=((TestTimeMonitor) vt.get(vDataLoop)).getAll_mean()%></td>
				<%if(hilightMean == false){%>
				<td style="background-color:#22FF00">
				<%}else {%>
				<td>
				<%}%><%=((TestTimeMonitor) vt.get(vDataLoop)).getAll_meanMaxByTester()%></td>
			...
			<%} else if (reportByPasserDrives.equals("checked")){%>
				...
				<%}else {%>
				<td>
				<%}%><%=((TestTimeMonitor) vt.get(vDataLoop)).getPasser_mean()%></td>
				<%if(hilightMean == false){%>
				<td style="background-color:#22FF00">
				<%}else {%>
				<td>
				<%}%><%=((TestTimeMonitor) vt.get(vDataLoop)).getPasser_meanMaxByTester()%></td>
