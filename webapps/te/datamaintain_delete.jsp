...
<head>
	...
	%>
	 ...
	 <%
	      String datamaintain =request.getParameter("maintainselect");    	      
                ...
                <table width="100%" border="0">
                  <tr>
                    ...
                    <td width="96%"><table width="100%" border="0" align="left">
                      ...
                      <p>&nbsp;</p>
                      <p>
                        <input name="DeleteData" type="button" id="DeleteData" value="Delete data" onclick="clickDelete()"/>                        
                        <input name="Cancel" type="button" id="Cancel" value="Cancel" onclick="CancelDelete()" />
                      </p></td>
                    <td width="10px">&nbsp;</td>
                  </tr>
                  <tr>
                    ...
                    <td> <table width="100%" border="0">
						...
						<%
                           Vector fieldList = dmtt.getFieldList(); 
							...
							%><td width="1%" height="10" align="center" class="style4"></td><%
		               ...
		               %> 
                         ...
                         <% 
						  for(int jj=0;jj<acvcv[0].size();jj++){						  
                                ...
                                for(int ii=0;ii<acvcv.length;ii++){ 									
									tmp=tmp+acvcv[ii].get(jj)+"~~";
								}
                                %><td width="1%" height="1%" align="right" class="style4"><input name="deleteData" type="checkbox" value="<%=tmp%>"></td><%

								for(int ii=0;ii<acvcv.length;ii++){ 									
									 %>
									 <td width="<%out.println(colwidth1);%>%" height="1%"><span class="style4"><%out.println(acvcv[ii].get(jj)); %></span></td>				 
									<%
								}
								%></tr><%
							}            
                        ...
                        <tr>
						 ...
						 <tr> 
                          <td colspan="10"> 						    
                            <input name="datamaintain" type="hidden" id="datamaintain"  value="<%=datamaintain%>"  />
							<input name="maintainevent" type="hidden" id="maintainevent"  value="deletedata"  />
                          </td>
                        </tr>
                      </table>
...
function clickDelete(){
	...
	var count=frm.deleteData.length;
	 if(typeof count == 'undefined' ){
	  count=1;
     }
	if(count>=2){
		  for(i=0;i<count;i++){
		   if (frm.deleteData[i].checked==true)
		   {
			   alertmsg="";
		   }	   
		}
	}else{		
	   if (frm.deleteData.checked==true)
	  {
		   alertmsg="";
	   }
	}
	    
	for(i=0;i<frm.deleteData.length;i++){
	   if (frm.deleteData[i].checked==true)
	   {
		   alertmsg="";
	   }
   ...
   if(alertmsg == ""){
		frm.action = "datamaintain_process.jsp";		
